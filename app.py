...
st.markdown(
    ...
    <div class="section-card">
        <div class="section-title">Upload a CSV dataset</div>
        <div class="section-subtitle">The agent will profile your data and generate tailored business insights.</div>
    """,
    unsafe_allow_html=True
)
...
if uploaded_file is not None:
    ...
    if is_car_data:
        ...
        with col1:
            premium_widget("Market Volume", f"₪{total_market_value:.1f}M", f"Across {len(df):,} listings", "linear-gradient(135deg, #FF9A9E 0%, #FECFEF 99%, #FECFEF 100%)")
        ...
        with col2:
            premium_widget("Most Popular", top_car_name, "Highest number of listings", "linear-gradient(135deg, #667EEA 0%, #764BA2 100%)")
        ...
        with col3:
            premium_widget("Best Investment", best_investment_name, f"Retains {best_retention_val:.1f}% of value", "linear-gradient(135deg, #43E97B 0%, #38F9D7 100%)")
            
    else:
        # Generic dataset fallback
        with col1:
            premium_widget("Total Records", f"{len(df):,}", "Rows in dataset", "linear-gradient(135deg, #667EEA 0%, #764BA2 100%)")
        with col2:
            premium_widget("Features", f"{len(df.columns)}", "Columns in dataset", "linear-gradient(135deg, #FF9A9E 0%, #FECFEF 100%)")
        with col3:
             premium_widget("File Size", f"{(df.memory_usage(deep=True).sum() / (1024*1024)):.2f} MB", "Memory footprint", "linear-gradient(135deg, #43E97B 0%, #38F9D7 100%)")
